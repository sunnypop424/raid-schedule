...
- 첫 진입 시 **"닉네임을 선택해 주세요"** 화면에서 8명 중 1명을 선택. (UI 문구는 전부 합니다/입니다/주세요체 — DESIGN.md §6)
- 선택값은 `localStorage`에 저장 → 다음 방문 시 자동 진입.
- 내 일정 카드 머리의 **"닉네임 변경"** 버튼으로 언제든 변경 가능. (헤더에는 주차 이동만 둔다)
...
- 선택한 주차의 **수·목·금·토·일·월·화 7일**을 한 화면(스크롤 없음)에 표 형태로 표시: `날짜 | 가능 여부 | 시간`.
- **가능 여부**는 날짜마다 셀렉트로 고른다. 기본값은 `- 선택 -`.

| 상태 | 값 | 색상 가이드 | 의미 |
|---|---|---|---|
| 가능 | `available` | 초록 ○ | 그날 레이드 가능 (시작 시간 선택) |
| 불가능 | `unavailable` | 빨강 ✕ | 그날 불가 |
| 미정 | `undecided` | 노랑 △ | 아직 모름 (제출로는 인정) |
| 미입력 | `null` (row 없음) | 회색 – | `- 선택 -` 상태 |

- **시간**은 날짜별로 따로 고른다. 상태가 **가능**일 때만 활성화.
  - 옵션: `종일`(기본) + `00:00 이후` ~ `23:30 이후`, **30분 단위**.
  - 저장값 `start_minutes`: 그날 00:00 기준 분(0~1410). `null` = 종일.
- 빠른 입력 버튼: **"전부 가능"**(종일), **"전부 불가능"**, **"지난주와 동일"**(상태+시간 복사).
- 변경 즉시 자동 저장 (디바운스 300ms) + 저장 상태 표시("저장됨 ✓").

### 3.2 공유 뷰 (주간 전체 현황)

- 해당 주차 7일을 **날짜별 카드**로 표시. 카드 한 장 = 날짜 · 8칸 현황 막대(가능→미정→불가능→미입력 순) · 인원 요약.
- 날짜별 **가능 인원**과 나머지를 따로 표기: `가능 5 · 미정 1 · 불가 2`. 미정은 가능 인원에 넣지 않는다.
- **모이는 시각**: 가능한 사람들의 시작 시간 중 가장 늦은 값. `4인 가능 · 22:00부터` (전원 종일이면 `종일 가능`).
- **레이드 추천일 하이라이트**
  - 8/8 가능 → 카드 전체를 퍼플 반전 (8인 레이드 가능)
  - 4명 이상 가능 → 연한 퍼플 테두리 (4인 레이드 가능)
  - 기준 인원(4, 8)은 설정 상수로 관리.
- 카드를 누르면 펼쳐져서 가능한 사람은 `닉네임 + 시간` 칩으로, 미정·불가능·미입력은 이름 한 줄로 보여준다.
- 상단에 `제출 6/8` 진행 표시와 **"미제출: 항상그놈, 흑마66"**.
...
- **기본 진입 주차는 다음 주차**(지금 제출해야 할 주차). 진행 중인 주차의 수정은 `◀ 이전 주`로 이동해서 한다(오늘 이후 날짜만 수정 가능).
- 다른 주차를 보고 있을 때 "다음 주 일정으로" 돌아가기 버튼.

### 3.4 제출 현황

- 멤버가 해당 주 7일을 모두 입력하면(미정 포함) **제출 완료**.
...
- 이미 레이드 약속을 잡은 날을 불가능으로 바꾸는 경우를 대비해, 저장 전 확인 모달: "이미 공유된 일정을 변경합니다. 공대원에게도 따로 알려 주세요." (P1)
...
| 경로 | 화면 |
|---|---|
| `/` | 사용자 미선택 시 → `/select`, 선택 시 → `/week/me` (내 일정 입력이 첫 화면) |
| `/select` | 멤버 선택 |
| `/week/me?w=YYYY-MM-DD` | 내 일정 입력 (기본 탭 · 이름 선택 직후 진입) |
| `/week?w=YYYY-MM-DD` | 공유 뷰 |

### 5.2 레이아웃

- 상단: 주차 네비게이션(◀ 주차 ▶)만. 현재 사용자·닉네임 변경은 내 일정 카드 머리에 둔다
- 하단 탭바(모바일): **내 일정 / 전체 일정** (내 일정이 첫 번째)
- 데스크톱(≥1024px): 좌측 내 일정 입력 패널 + 우측 공유 그리드 2단 구성

### 5.3 모바일 대응 (필수)

- 기준 뷰포트: 360px ~ 430px 우선 설계, 이후 태블릿/데스크톱 확장.
- **공유 뷰 레이아웃:** 모바일·데스크톱 모두 날짜별 카드 리스트(§3.2). 데스크톱은 카드 한 줄에 `날짜 | 막대 + "닉네임 시간" 한 줄 | 큰 숫자`를 가로로 배치.
- 터치 타겟 최소 44×44px.
- 하단 탭바는 `env(safe-area-inset-bottom)` 대응.
- 내 일정 입력은 네이티브 셀렉트 2개(가능 여부 / 시간)로 한 손 조작.
...
create type availability_status as enum ('available', 'unavailable', 'undecided');
...
create table availabilities (
  ...
  start_minutes smallint              -- 가능 시작 시간(분), null = 종일. 0(00:00)~1410(23:30), 30분 단위
...
- **수정 규칙 강제:** `set_availability(member_id, date, status, start_minutes)` RPC를 만들고 함수 내에서 `date < (now() at time zone 'Asia/Seoul')::date`이면 예외(지난 날짜 수정 불가). 마감 이후 변경이면 `availability_logs`에 이력 기록. 테이블 직접 쓰기는 RLS로 막고 RPC만 허용.
...
3. ~~시간대 입력~~ → **확정:** 날짜별 시작 시간 셀렉트(종일 / 00:00~23:30, 30분 단위). 메모 없음
4. PIN 보호 도입 시점
...
3. **Phase 3 — 내 일정 입력:** 7일 표(날짜·가능 여부 셀렉트·시간 셀렉트), 자동 저장, 빠른 입력 버튼
4. **Phase 4 — 공유 뷰:** 날짜별 카드(현황 막대·펼침), 가능 인원/모이는 시각, 추천일 하이라이트, 미제출자 표시
